...
- **路由中枢**：`registry.json` 统一登记工作目录文件、脚本（用途/用法/退出码）与七大功能（触发词/参数/上下文/执行链）；`route.cmd` 冷启动识别意图并给出执行链，`-Check` 一键自检舒展是否漂移。
...
├── VERSION                  # 语义化版本号（如 0.4.0）
├── registry.json            # 路由中枢注册表（唯一事实源）：文件 / 脚本 / 功能意图 / 路由规则
├── scripts/
│   ├── route.*              # 路由中枢：意图识别、上下文装配、执行链、一致性自检（.ps1 + .cmd）
...
```bat
route.cmd -Guess "帮我查一下之前自我介绍怎么写的"  :: 意图识别 + 上下文与执行链
